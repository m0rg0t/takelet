...
Optional ElevenLabs generation sends the selected script and voice/model/language
settings to ElevenLabs and consumes the user's provider credits. The API key is
stored only in device-local macOS Keychain. Generated MP3s and custom cursor PNGs
are copied into projects; scripts and previous audio takes may remain in a package
after editing, so inspect the whole package before sharing it.
